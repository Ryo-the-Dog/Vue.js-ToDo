...
var bus = new _vue2.default(); // 公式を自分でアレンジ
//　https://jp.vuejs.org/v2/examples/todomvc.html

var bus2 = new _vue2.default();
var bus3 = new _vue2.default();
var bus4 = new _vue2.default();
// 追加欄用子コンポーネント
// コンポーネントで使う場合のdataは必ず関数にすること！
_vue2.default.component('add-todo', {
    data: function data() {
        return {
            newTodo: '', // 追加するTODO内容
            uid: 0,
    ...
    computed: {
        filteredTodos: function filteredTodos() {
            var todos = [];
            var regexp = new RegExp('^' + this.catchedSearchText, 'i');
    ...
    methods: {
        addTodo: function addTodo() {
            ...
            var value = this.newTodo && this.newTodo.trim(); // trim()で文字列両端の空白を削除する。
            if (!value) {
                // 空白の場合
                this.canAddTodo = '';
        ...
        // 日本語変換の時のエンターを制御
        setCanAddTodo: function setCanAddTodo() {
            this.canAddTodo = true;
        },

        // 検索欄に入力された文字を取得。そして検索に掛けた後のTODO群を一覧表示コンポーネントに渡す。
        catchSearchText: function catchSearchText(searchText) {
            this.catchedSearchText = searchText;
            bus3.$emit('bus-third-event', this.filteredTodos);
        },

        // 削除処理後のTODO群を取得
        catchRemovedTodo: function catchRemovedTodo(removedTodo) {
            this.todos = removedTodo;
        }
    }
});
new _vue2.default({ // 子コンポーネントを生み出すための親インスタンスを生成する
    el: '#add-todo'
});
//検索欄用コンポーネント
_vue2.default.component('search-todo', {
    data: function data() {
        return {
    ...
    methods: { // computedはドキュメントにも書いてありますが、高速化 and 省力化のためですね。
        ...
        throwSearchText: function throwSearchText() {
...
_vue2.default.component('todo-list', {
    ...
    methods: {
        ...
        catchFirstTodos: function catchFirstTodos(firstTodos) {
            this.filteredTodos = firstTodos;
        },
        // 検索でフィルターに掛けられたTODO群をこのコンポーネント内で編集できるように格納する。
        catchTodos: function catchTodos(filteredTodos) {
